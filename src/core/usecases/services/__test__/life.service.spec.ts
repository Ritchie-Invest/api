...
describe('LifeService', () => {
  ...
  beforeEach(() => {
    ...
    lifeService = new LifeService(userRepository, mockPrisma);
  });
  ...
  it('should return has_lost=true when all 5 lives lost in last hour', async () => {
    ...
    it('should show 4 lives then 5 lives after 1h passes for a single lost life', async () => {
      ...
      mockPrisma.life.findFirst.mockResolvedValueOnce({ emissionDate: lost30MinAgo });
    ...
    it('should handle multiple lost lives with staggered regeneration timings', async () => {
      ...
      mockPrisma.life.findFirst.mockResolvedValueOnce({ emissionDate: lost40MinAgo });
      ...
      mockPrisma.life.findFirst.mockResolvedValueOnce({ emissionDate: lost10MinAgo });
  ...
  describe('addLostLife', () => {
    it('should call userRepository.addLostLife with correct userId', async () => {
      // Given
      const userId = 'user-1';
      const addLostLifeSpy = jest.spyOn(userRepository, 'addLostLife').mockResolvedValue();
